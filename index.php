...
if (!is_null($events['events'])) {
	...
	foreach ($events['events'] as $event) {
		...
		if ($event['type'] == 'message' && $event['message']['type'] == 'text') {
			...
			$array_result_array = array('ไม่มีสมองพิมพ์เองหรือไง ขี้ก๊อป อายคนอื่นเขาบ้างเถอะ ','ว๊าย ขี้ก็อป','บ้านอยู่ต่างประเทศเปล่าเนี่ย จีนแดงอ่ะนะ','ไม่มีสมอง ก็อปคนอื่นไปเรื่อย','อุ้ย ควาย ก๊อป');
			$output_result_array = $array_result_array[array_rand($array_result_array,1)];
			
			$result_array   = array_search($text,$array_result_array); 
			...
			$arrayyyyre = array('คนพิมพ์ก็เช่นกัน','คนพิมพ์ก็เช่นกัน','คนพิมพ์ก็เช่นกัน','คนพิมพ์ก็เช่นกัน','คนอ่านก็เช่นกัน','คนพิมพ์ก็เช่นกัน','ผมเตือนคุณแล้วนะ อย่าให้ผมต้องเรียกคุณว่า ไอ้กาก เลยครับ นะครับ ไอ้กาก หยุดนะครับ', 'หยุดเถอะ คนอื่นเค้าจะรำคาญเอานะ', 'อย่าว่าคนอื่นแบบนั้นสิ พ่อแม่ที่บ้าน ไม่เคร่งเรื่องมารยาทสินะ ถึงได้พิมพ์แบบนี้','ใจเย็น ๆ ค่อย ๆ พิมพ์', 'ใจเย็ด ๆ  ค่อย ๆ พิมพ์ เอ้ย ใจเย็น ๆ ค่อย ๆ พิมพ์', 'กรุณาวางแป้นพิมพ์ แล้วตั้งสติ', 'กรุณาวางถุงกาวลงเเละตั้งสติ', 'กากทั้งวันทั้งคืน เสพติดความกากสินะ','อย่าด่าว่า กาก เดี๋ยวปากจะร้าวเอาได้นะ', 'ถามใจเธอดู เบื่อไหม กับคำว่า กาก','คนพิมพ์นี่ สงสัยบ้านไม่มีกระจกเนอะ ว่าง ๆ ตักน้ำใส่กะโหลก ชะโงกดูเงาตัวเอง บ้างนะ กาก ติดกลางหน้าผากเลย','กากทุกคนนั้นแหละ ไม่ต้องแย่งกัน แบ่ง ๆ กันไป','กากมาก ก. ไก่ ล้านตัว','กาก จริง ๆ เนอะ','เห็นด้วย 100 % ครับ เจ้านาย','คนพิมพ์นั่นแหละ กาก','ปุ่มไลค์ อยู่ตรงไหน ไม่ได้จะกดไลน์นะ แต่จะกระทืบไลค์เลย','กดไลค์ให้ 1 ที ถูกใจเล็กน้อย','ว่าแต่ กาก คืออัลลัย บอทไม่เข้าใจ','เก็บความกากไว้ใช้เองเถอะ','อย่ายัดเยียดความกากให้คนอื่นเลย','ว่าแต่คนอื่น อิเหนาเป็นเอง เคยได้ยินม่ะ','คำนี้ ผมขอเถอะ อย่าพิมพ์อีกนะ','ต้องเลวขนาดไหน ถึงกล้าพิมพ์แบบนี้','ใช่ กากมาก คนที่พิมพ์อ่ะนะ','ใช่ กากมาก','ช่าย กากมาก','เรียนมาน้อยสินะ เลยพิมพ์ได้แค่คำว่า กาก', 'ปมด้อยสินะ ถึงว่าคนอื่นกาก', 'ว่าคนอื่นกาก ก็กากเหมือนกันแหละว้า', 'กาก กาก กาก กาก กาก กาก กาก กาก กาก พอใจม่ะ', 'อย่าหาว่าสอนเลยนะ คำว่า กาก เค้าไว้ใช้เรียกคุณนั่นแหละ', 'ไม่ได้ไปกากบนหัวคุณสักหน่อย', 'พิมพ์คำอื่นไม่เป็นหรือไง', 'กากแล้วไง ไปยุ่งกับเค้าทำไม', 'ใครกันแน่ที่กาก', 'มั่นใจว่ากาก', 'กากแล้วทำไม', 'พ่อแม่ไม่รักสินะ ถึงว่าคนอื่น', 'กาก แล้วไง', 'มีปัญหาป่ะ', 'กาก ก๊าก กาก', 'กากจริง ๆ', 'เดินไปส่องกระจกสิ จะเจอกากกว่า', 'กอ อา กอ กาก', 'ขอบคุณ');
			
			$result_array   = array_search($text, $arrayyyyre); 
			...
			$arraybare = array('ก็บ้า เพราะคุยกับคุณนั่นแหละ', 'ก็บ้าเหมือนกันนั่นแหละ คนอะไรคุยกับบอท', 'ศรีธัญญาไปทางไหน ไปด้วยกันไหม', 'ฉีดยาแล้ว ไม่บ้าหรอก ว่าแต่คุณฉีดหรือยัง ?', 'กรุณาวางถุงกาวลงเเละตั้งสติ');
			
			$result1_array   = array_search($text,$arraybare); 
